...
We find that agricultural workers experienced monthly earnings gains of 3--5 percent relative to other sectors following the embargo. Event study analysis shows these effects emerged after 2014 and grew over time, with pre-treatment coefficients close to zero. Effects are concentrated in high-agricultural-intensity regions. However, dose-response tests find no evidence that effects are larger in regions with greater exposure to banned products---if anything, the pattern is reversed---which weakens the case that import substitution is the primary mechanism.
